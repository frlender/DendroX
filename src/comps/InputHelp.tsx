...
export default function InputHelp(){
    ...
    return <div className='help-container'>
        ...
        <div className='help-sub-row'> <span className="row-head">1)</span> An input file that describes a dendrogram in JSON format. It can be generated in <span className="row-head">two</span> ways:</div>
        <div className='help-sub-sub-row'> <span className="row-head">a)</span> Using the R or Python <a target='_blank' rel='noreferrer' href='https://github.com/frlender/DendroX/tree/main/input_demo'><span className="row-head">get_json</span></a> function provided by us. The function accepts return value from the Python <a target='_blank' rel='noreferrer' href='https://seaborn.pydata.org/generated/seaborn.clustermap.html'>seaborn.clustermap</a> or the R <a target='_blank' rel='noreferrer' href='https://www.rdocumentation.org/packages/pheatmap/versions/1.0.12/topics/pheatmap'>pheatmap</a> function as argument. Demos and example input files could be found  <a target='_blank' rel='noreferrer' href='https://github.com/frlender/DendroX/tree/main/input_demo'>here</a>. R and Python packages that wrap the get_json function can be found  <a target='_blank' rel='noreferrer' href='https://github.com/frlender/dendrox-util'>here</a>.</div>
        
        <div className='help-sub-sub-row'> <span className="row-head">b)</span> Using the <a target='_blank' rel='noreferrer' href='https://github.com/frlender/dendrox-cluster'><span className="row-head">DendroX Cluster</span></a> program developed by us. It provides a graphic user interface to generate JSON and image inputs to DendroX from a data matrix stored in a text file. It should be downloaded and run locally on your computer.</div>
        
        <div className='help-sub-row extra-margin'> <span className="row-head">2)</span> A session file that describes an array of dendrograms in JSON format. The session file is generated by clicking on the download session button in the visualization view of DendroX.</div>
        ...
        <div className="help-row mt-1"><span className="row-head">L1000 example:</span> Example of a dendrogram that clusters 297 bioactive LINCS L1000 compounds. (Link to input files: <a target='_blank' rel='noreferrer' href='https://github.com/frlender/DendroX/blob/main/public/L1000.json'>json</a>, <a target='_blank' rel='noreferrer' href='https://github.com/frlender/DendroX/blob/main/public/L1000.png'>png</a>.) </div>

        <div className="help-row mt-neg"><span className="row-head">Genes example:</span> Example of a dendrogram that clusters a gene expression matrix with 600 genes. (Link to input files: <a target='_blank' rel='noreferrer' href='https://github.com/frlender/dendrox-app/blob/main/genes.json'>json</a>, <a target='_blank' rel='noreferrer' href='https://github.com/frlender/dendrox-app/blob/main/600_gene_cropped.png'>png</a>.)</div>
        ...
        <div className="help-row mt-1">A session could be also saved in the browser's local storage by clicking on the save session button in the visualization view. After a session is saved, it could be retrieved in this view by clicking on the session link. The session links are located below the JSON input element and in the format of: <span className="session-format">Session name  [year/month/day hour:minute]</span></div>
